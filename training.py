        ...
        for epoch in range(num_epochs):
            ...
            for inputs, labels in train_loader:
                ...
                for i in range(inputs.shape[0]):
                    ...
                    running_corrects_train += torch.all(labels_row[preds_row])
                    # if torch.all(labels_row[preds_row]):
                    #     print(torch.all(labels_row[preds_row]))
                    #     print(preds_row)
                    #     print(labels_row)
                    # print(torch.all(labels_row[preds_row]))

                print(running_corrects_train)
                exit()
            ...
            for inputs, labels in val_loader:
                ...
                for i in range(inputs.shape[0]):
                    ...
                    running_corrects_val += torch.all(labels_row[preds_row])
